...
public class RobotContainer {
  ...
  private static LoggedDashboardChooser<Command> m_driverChooser =
      new LoggedDashboardChooser<>("DriverChooser");
  ...
  private void configureBindings() {
    ...
    Objects.DRIVER_CONTROLLER
        .b()
        .whileTrue(m_robotArm.MoveArmCommand("HIGH", false))
        .whileFalse(new InstantCommand(() -> {}));

    Objects.DRIVER_CONTROLLER
        .a()
        .whileTrue(Commands.runOnce(() -> m_robotArm.setSpeeds(6.0), m_robotArm))
    ...
    buttonArmToSetpoint(Objects.OPERATOR_CONTROLLER.button(Map.B_ARM_HIGH), "HIGH");

    buttonArmToSetpoint(Objects.OPERATOR_CONTROLLER.button(Map.B_ARM_MID), "MID");

    buttonArmToSetpoint(Objects.OPERATOR_CONTROLLER.button(Map.B_ARM_LOW), "LOW");

    buttonArmToSetpoint(Objects.OPERATOR_CONTROLLER.button(Map.B_ARM_SUB), "SUB");

    buttonArmToSetpoint(Objects.OPERATOR_CONTROLLER.button(Map.B_ARM_GROUND), "GRND");

    buttonArmToSetpoint(Objects.OPERATOR_CONTROLLER.button(Map.B_ARM_IDLE), "IDLE");

    buttonManualArm(Objects.OPERATOR_CONTROLLER.button(Map.B_ARM_IN_MANUAL), 0.5);

    buttonManualArm(Objects.OPERATOR_CONTROLLER.button(Map.B_ARM_OUT_MANUAL), -0.5);

    buttonRunIntake(Objects.OPERATOR_CONTROLLER.button(Map.B_INTAKE_IN), true);

    buttonRunIntake(Objects.OPERATOR_CONTROLLER.button(Map.B_INTAKE_OUT), false);
  ...
  private void buttonArmToSetpoint(Trigger button, String position) {
    button.whileTrue(m_robotArm.MoveArmCommand(position, false)).whileFalse(new InstantCommand());
  }
  ...
  private void buttonManualArm(Trigger button, double speed) {
    button
        .whileTrue(
            Commands.runOnce(
                ((Vars.IS_SNIPER)
                    ? () -> m_robotArm.setSpeeds(speed * 0.5)
                    : () -> m_robotArm.setSpeeds(speed)),
                m_robotArm))
        .whileFalse(new InstantCommand());
  }
  ...
  private void buttonRunIntake(Trigger button, boolean isIntaking) {
    button
        .whileTrue(
            Commands.runOnce(
                isIntaking ? () -> m_robotIntake.intake(null) : () -> m_robotIntake.outtake(null),
                m_robotIntake))
        .whileFalse(new InstantCommand());
  }
  ...
  private void buttonSniperMode(Trigger button) {
    button
        .whileTrue(Commands.runOnce(() -> Vars.IS_SNIPER = true, m_robotArm))
        .whileFalse(Commands.runOnce(() -> Vars.IS_SNIPER = false, m_robotArm));
  }
   ...
   * @return InstantCommand() that sets the profile instance var and calls configureBindings()
   */
