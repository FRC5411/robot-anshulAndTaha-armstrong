...
public class ArmSubsystem extends SubsystemBase {
  ...
  public void simulationPeriodic() {
    ...
    Simulation.SIM_ARM_VISUAL_MECH2D.setAngle(
        Units.radiansToDegrees(Simulation.SIM_ARM.getAngleRads()));
  ...
  public void setSpeeds(double volts) {
    ...
    Logger.getInstance()
        .recordOutput("/systems/arm/setSpeedsVolts", MathUtil.clamp(volts, -12.0, 12.0));
  }

  /**
   * Moves or holds the arm based on setpoints
   *
   * @param setpoint Setpoint to move the arm to (Will not do anything if isHolding is true)
  ...
  /**
   * Arm feedforward required the arm to be at a different idle angle than the robot is usually in.
